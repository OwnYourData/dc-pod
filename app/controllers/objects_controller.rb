class ObjectsController < ApplicationController
    ...
    def write
        ...
        retVal, status = write_object(id, payload, doorkeeper_org, doorkeeper_user)
        render json: retVal,
               status: status
    ...
    def read
        ...
        if show_meta.to_s == "TRUE"
            ...
            if store["schema"].to_s != ""
                retVal = retVal.merge({"schema" => store["schema"]})
            end
        else
            retVal = data
        end
        createEvent(meta["collection-id"], CE_READ_OBJECT, "read object", {object_id: store["id"], read_meta: (show_meta.to_s == "TRUE")}, doorkeeper_user)
